 ...
 * Tests queue-based behavior, constant leak rate, and traffic shaping
 * capabilities.
 */
public class LeakyBucketTest {
    ...
    @BeforeEach
    void setUp() {
        // Create leaky bucket with capacity 10, leak rate 2 tokens/second, max queue
        // time 5 seconds
    ...
    void testQueueCapacityLimit() {
        ...
        // Note: synchronous interface approximates behavior, actual queuing happens in
        // async mode
    }
    ...
    void testRequestTimeout() throws InterruptedException, ExecutionException, TimeoutException {
        // Create bucket with very short timeout and no leak rate (essentially no
        // processing)
        ...
        try {
            ...
            assertTrue(timeoutCount > 0 || completedCount < futures.size(),
                    "Some requests should have timed out or still be processing. Completed: " + completedCount
    ...
    void testConstantRateVsBurstBehavior() throws InterruptedException, ExecutionException, TimeoutException {
        // Test that leaky bucket provides constant output rate regardless of input
        // pattern
