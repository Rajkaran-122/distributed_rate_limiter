...
class GeographicRateLimitServiceTest {
    ...
    void testCheckGeographicRateLimit_WithGeographicRule() {
        ...
        // When
        GeographicRateLimitResponse response = geographicRateLimitService.checkGeographicRateLimit(key, sourceIP,
                tokens, headers);
    ...
    void testCheckGeographicRateLimit_WithRateLimitRequest() {
        ...
        // When
        GeographicRateLimitResponse response = geographicRateLimitService.checkGeographicRateLimit(request, sourceIP,
                headers);
    ...
    void testCheckGeographicRateLimit_WithClientInfoOverride() {
        ...
        // When
        GeographicRateLimitResponse response = geographicRateLimitService.checkGeographicRateLimit(request,
                originalSourceIP, originalHeaders);
        ...
        verify(geoLocationService).detectLocation(eq("203.0.113.1"),
                argThat(headers -> headers.containsKey("CF-IPCountry") &&
    ...
    void testCreateGeographicKey() {
        ...
        // Then
        // Verify that the geographic key is created properly (starts with geo: and
        // contains country and compliance zone)
        verify(rateLimiterService).isAllowed(argThat(geographicKey -> geographicKey.startsWith("geo:DE:GDPR:") &&
                geographicKey.endsWith(":" + key)), eq(tokens));
    }
}