 ...
 * Orchestrates geographic location detection, rule resolution, and rate limit
 * enforcement.
 */
@Service
...
public class GeographicRateLimitService {
    ...
    public GeographicRateLimitResponse checkGeographicRateLimit(String key, String sourceIP,
        ...
        try {
            ...
            if (geoLocation == null) {
                logger.debug("Could not detect geographic location for IP: {}, falling back to standard rate limiting",
                        sourceIP);
            ...
            if (geographicConfig == null) {
                logger.debug(
                        "No geographic rules apply for key: {} location: {}, falling back to standard rate limiting",
            ...
            try {
                ...
                return new GeographicRateLimitResponse(key, tokens, allowed, geoLocation, appliedRule,
                        geographicConfig);
            } finally {
     ...
     * This ensures that different geographic regions have separate rate limit
     * buckets.
     */
