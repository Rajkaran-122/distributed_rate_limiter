...
import com.Rajkaran_122.distributedratelimiter.models.PerformanceRegressionResult.RegressionSeverity;
import com.Rajkaran_122.distributedratelimiter.models.PerformanceRegressionResult.RegressionThresholds;
...
public class PerformanceRegressionService {
    ...
    public PerformanceRegressionService(
            @Value("${performance.baseline.storage.path:./target/performance-baselines.json}") String baselineStoragePath) {
        ...
        try {
            ...
            if (responseTimeIncrease > thresholds.getResponseTimeThreshold()) {
                ...
                String detail = String.format("Response time increased by %.1f%% (%.2fms -> %.2fms)",
                        responseTimeIncrease, previousBaseline.getAverageResponseTime(),
                        currentBaseline.getAverageResponseTime());
                regressionDetails.add(detail);
                maxSeverity = getWorstSeverity(maxSeverity,
                        getSeverityFromIncrease(responseTimeIncrease, thresholds.getResponseTimeThreshold()));
            }
            ...
            if (throughputDecrease > thresholds.getThroughputThreshold()) {
                ...
                String detail = String.format("Throughput decreased by %.1f%% (%.2f -> %.2f req/sec)",
                        throughputDecrease, previousBaseline.getThroughputPerSecond(),
                        currentBaseline.getThroughputPerSecond());
                regressionDetails.add(detail);
                maxSeverity = getWorstSeverity(maxSeverity,
                        getSeverityFromIncrease(throughputDecrease, thresholds.getThroughputThreshold()));
            }
            ...
            if (successRateDecrease > thresholds.getSuccessRateThreshold()) {
                ...
                regressionDetails.add(detail);
                maxSeverity = getWorstSeverity(maxSeverity,
                        getSeverityFromIncrease(successRateDecrease, thresholds.getSuccessRateThreshold()));
            }
    ...
    private RegressionSeverity getSeverityFromIncrease(double increase, double threshold) {
        if (increase < threshold * 1.5)
            return RegressionSeverity.MINOR;
        if (increase < threshold * 2.0)
            return RegressionSeverity.MODERATE;
        if (increase < threshold * 3.0)
