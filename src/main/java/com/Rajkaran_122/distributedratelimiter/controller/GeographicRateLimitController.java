...
import com.Rajkaran_122.distributedratelimiter.models.GeoLocation;
import com.Rajkaran_122.distributedratelimiter.models.GeographicRateLimitConfig;
...
/**
 * REST controller for managing geographic rate limiting rules and debugging
...
public class GeographicRateLimitController {
    ...
    @GetMapping("/rules")
    @Operation(summary = "Get all geographic rate limiting rules", description = "Returns all configured geographic rate limiting rules")
    @ApiResponses(value = {
    ...
    @PostMapping("/rules")
    @Operation(summary = "Add a new geographic rate limiting rule", description = "Creates a new geographic rate limiting rule")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Remove a geographic rate limiting rule", description = "Removes a geographic rate limiting rule by ID")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Rule removed successfully"),
            @ApiResponse(responseCode = "404", description = "Rule not found")
    })
    public ResponseEntity<String> removeRule(
            @Parameter(description = "Rule ID to remove") @PathVariable String ruleId) {
    ...
    @GetMapping("/detect")
    @Operation(summary = "Detect geographic location for current request", description = "Detects and returns geographic location information for debugging purposes")
    @ApiResponses(value = {
    ...
    public ResponseEntity<Map<String, Object>> detectLocation(HttpServletRequest request,
        ...
        try {
            ...
            if (location != null) {
                ...
                List<GeographicRateLimitConfig> applicableRules = geographicConfigResolver
    ...
    @GetMapping("/stats")
    @Operation(summary = "Get geographic rate limiting statistics", description = "Returns cache statistics and performance metrics for geographic rate limiting")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Clear geographic caches", description = "Clears all geographic location and configuration caches")
    @ApiResponses(value = {
