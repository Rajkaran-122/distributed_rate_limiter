...
import com.Rajkaran_122.distributedratelimiter.models.AdaptiveStatus;
...
public class AdaptiveRateLimitController {
    ...
    @Operation(summary = "Get adaptive status", description = "Retrieve adaptive rate limiting status for a specific key including current limits, recommendations, and reasoning")
    public ResponseEntity<AdaptiveStatus> getAdaptiveStatus(
            @Parameter(description = "Rate limit key", required = true) @PathVariable String key) {
    ...
    @Operation(summary = "Set manual override", description = "Manually override adaptive decisions for a specific key")
    public ResponseEntity<Void> overrideAdaptation(
        ...
        AdaptiveRateLimitEngine.AdaptationOverride engineOverride = new AdaptiveRateLimitEngine.AdaptationOverride(
                override.capacity,
                override.refillRate,
                override.reason);
    ...
    @Operation(summary = "Remove manual override", description = "Remove manual override and resume adaptive rate limiting for a key")
     ...
     */
    @GetMapping("/config")
    @Operation(summary = "Get adaptive configuration", description = "Retrieve current adaptive rate limiting configuration")
