...
import com.Rajkaran_122.distributedratelimiter.models.RateLimitResponse;
import com.Rajkaran_122.distributedratelimiter.models.CompositeRateLimitResponse;
import com.Rajkaran_122.distributedratelimiter.models.GeographicRateLimitResponse;
import com.Rajkaran_122.distributedratelimiter.ratelimit.RateLimiterService;
import com.Rajkaran_122.distributedratelimiter.ratelimit.CompositeRateLimiterService;
...
import com.Rajkaran_122.distributedratelimiter.security.ApiKeyService;
import com.Rajkaran_122.distributedratelimiter.security.IpAddressExtractor;
import com.Rajkaran_122.distributedratelimiter.security.IpSecurityService;
...
public class RateLimitController {
    ...
    @PostMapping("/check")
    @Operation(summary = "Check rate limit for a key", description = "Checks if a request is allowed based on the configured rate limits for the given key. Supports geographic rate limiting when enabled.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Request allowed", content = @Content(mediaType = "application/json", schema = @Schema(implementation = RateLimitResponse.class), examples = @ExampleObject(value = "{\"key\":\"user:123\",\"tokensRequested\":1,\"allowed\":true}"))),
            @ApiResponse(responseCode = "429", description = "Rate limit exceeded", content = @Content(mediaType = "application/json", schema = @Schema(implementation = RateLimitResponse.class), examples = @ExampleObject(value = "{\"key\":\"user:123\",\"tokensRequested\":1,\"allowed\":false}"))),
            @ApiResponse(responseCode = "401", description = "Invalid API key", content = @Content(mediaType = "application/json", schema = @Schema(implementation = RateLimitResponse.class))),
            @ApiResponse(responseCode = "403", description = "IP address not allowed", content = @Content(mediaType = "application/json", schema = @Schema(implementation = RateLimitResponse.class)))
    })
    public ResponseEntity<RateLimitResponse> checkRateLimit(
            @Parameter(description = "Rate limit request containing key, tokens, optional API key, and optional client info for geographic rate limiting", required = true, content = @Content(examples = @ExampleObject(value = "{\"key\":\"user:123\",\"tokens\":1,\"apiKey\":\"your-api-key\",\"clientInfo\":{\"sourceIP\":\"192.168.1.1\",\"countryCode\":\"US\"}}"))) @Valid @RequestBody RateLimitRequest request,
