...
import com.Rajkaran_122.distributedratelimiter.models.EmergencyScheduleRequest;
import com.Rajkaran_122.distributedratelimiter.models.ScheduleRequest;
import com.Rajkaran_122.distributedratelimiter.models.ScheduleResponse;
import com.Rajkaran_122.distributedratelimiter.ratelimit.RateLimitConfig;
import com.Rajkaran_122.distributedratelimiter.schedule.RateLimitSchedule;
import com.Rajkaran_122.distributedratelimiter.schedule.ScheduleManagerService;
import com.Rajkaran_122.distributedratelimiter.schedule.ScheduleType;
...
public class ScheduleController {
     ...
     */
    @PostMapping
    @Operation(summary = "Create a new rate limit schedule", description = "Creates a new scheduled rate limit configuration for recurring, one-time, or event-driven adjustments")
     ...
     */
    @GetMapping
    @Operation(summary = "Get all rate limit schedules", description = "Retrieves all configured rate limit schedules with their active status")
    @ApiResponse(responseCode = "200", description = "List of schedules retrieved successfully", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ScheduleResponse.class)))
     ...
     */
    @GetMapping("/{name}")
    @Operation(summary = "Get a specific rate limit schedule", description = "Retrieves a rate limit schedule by name")
     ...
     */
    @PutMapping("/{name}")
    @Operation(summary = "Update a rate limit schedule", description = "Updates an existing rate limit schedule configuration")
     ...
     */
    @DeleteMapping("/{name}")
    @Operation(summary = "Delete a rate limit schedule", description = "Removes a rate limit schedule from the system")
    ...
    @Operation(summary = "Activate a rate limit schedule", description = "Enables a previously deactivated schedule")
    ...
    @Operation(summary = "Deactivate a rate limit schedule", description = "Temporarily disables a schedule without deleting it")
    ...
    @Operation(summary = "Create emergency rate limit adjustment", description = "Creates a temporary rate limit adjustment for emergency situations (e.g., DDoS mitigation)")
