...
import com.Rajkaran_122.distributedratelimiter.models.AdminKeyStats;
import com.Rajkaran_122.distributedratelimiter.models.AdminKeysResponse;
import com.Rajkaran_122.distributedratelimiter.models.AdminLimitRequest;
import com.Rajkaran_122.distributedratelimiter.models.AdminLimitResponse;
...
import com.Rajkaran_122.distributedratelimiter.ratelimit.RateLimiterService;
...
public class AdminController {
    ...
    @Operation(summary = "Get rate limit configuration for a specific key", description = "Retrieves the current rate limiting configuration for the specified key")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Configuration found", content = @Content(mediaType = "application/json", schema = @Schema(implementation = AdminLimitResponse.class))),
            @ApiResponse(responseCode = "404", description = "Key configuration not found")
    })
    public ResponseEntity<AdminLimitResponse> getKeyLimits(
            @Parameter(description = "Rate limiting key", required = true, example = "user:123") @PathVariable("key") String key) {
    ...
    @Operation(summary = "Update rate limit configuration for a specific key", description = "Sets new rate limiting configuration for the specified key")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Configuration updated successfully", content = @Content(mediaType = "application/json", schema = @Schema(implementation = AdminLimitResponse.class)))
    })
    public ResponseEntity<AdminLimitResponse> updateKeyLimits(
            @Parameter(description = "Rate limiting key", required = true, example = "user:123") @PathVariable("key") String key,
            @Parameter(description = "New rate limit configuration", required = true) @Valid @RequestBody AdminLimitRequest request) {
        ...
        keyConfig
                .setCleanupIntervalMs(request.getCleanupIntervalMs() != null ? request.getCleanupIntervalMs() : 60000L);
        keyConfig.setAlgorithm(request.getAlgorithm() != null ? request.getAlgorithm()
                : com.Rajkaran_122.distributedratelimiter.ratelimit.RateLimitAlgorithm.TOKEN_BUCKET);
    ...
    @Operation(summary = "Remove rate limit configuration for a specific key", description = "Removes custom rate limiting configuration for the specified key, reverting to default settings")
    @ApiResponses(value = {
     ...
     */
    @GetMapping("/keys")
    @Operation(summary = "List all active rate limiting keys", description = "Retrieves statistics for all currently active rate limiting keys")
    @ApiResponse(responseCode = "200", description = "Active keys retrieved successfully", content = @Content(mediaType = "application/json", schema = @Schema(implementation = AdminKeysResponse.class)))
